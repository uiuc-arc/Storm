...
class PyroVisitor(Template2Visitor):
    ...
    def visitUnary(self, ctx=Template2Parser.UnaryContext):
    ...
    def visitTranspose(self, ctx=Template2Parser.TransposeContext):
        return "torch.t({0})".format(self.visit(ctx.expr()))
    ...
    functions = {
        ...
        "rep_val": "torch.tensor({0}).repeat([{1}])",
        ...
        "sum": "torch.sum({0})",
        "log10": "torch.log({0})",
        "rep_array": "torch.tensor({0}).repeat([{1}])",
        "to_vector": "{0}.view(-1)",
        "log1m": "torch.log(1-{0})",
        "rep_vector": "torch.tensor({0}).repeat([{1}])"
    }


class PyroTranslator(Template2Listener):
    def __init__(self, algo='SVI', iters=4000,samples=False, learning_rate=None):
    ...
    def create_program(self, templatefilepath, directory, filename):
        ...
        with open(directory + '/' + filename.split('/')[-1].replace('.template', '.py'), 'w') as modelfile:
    ...
    def enterPrior(self, ctx):
        ...
        for p in dist.params().param():
            if isinstance(p.expr(), Template2Parser.ValContext):
                if "1234.0" in dist.params().getText():
                    args.append("1.0")
                    args.append("10.0")
                    break
                args.append(
        ...
        for arg in prior['args']:
            if isinstance(arg, (int, float)):
                v = "torch.tensor({0})".format(arg)
                
            elif isinstance(arg, np.ndarray):
                v = "torch.tensor({0})".format(np.array2string(arg, separator=','))
        ...
        if 'pyro_args' in prior['prior']:
            curargs = ','.join(['{0}={1}*torch.ones([{2}])'.format(b['name'], a, dims) for a, b in
                                zip(argslist[:-1].split(','), prior['prior']['pyro_args'])])
        else:
            curargs = ','.join(['{0}*torch.ones([{1}])'.format(a, dims) for a in argslist[:-1].split(',')])
        ...
        if prior["prior"]["name"] == "categorical":
            ...
            posterior = np.random.choice([p for p in posterior_candidates if p['type'] == 'C'] + [prior["prior"]])
        arglist = ''
        args = []
        index = 0
        for arg in posterior["args"]:
            ...
            if arrdim is not None:
                indices = arrdim.count(',') + 1
                dimn = ",".join(dimn.split(",")[indices:])

            if "dim" in arg and arg["dim"] == "pdm":
                
                
                self.guide += self.guide_tab + "{0} = pyro.param('{1}', torch.eye(({2})))\n".format(
                    argname, argname, dimn.count(',') + 1)
            elif is_positive(arg["type"]):
                self.guide += self.guide_tab + "{0} = pyro.param('{1}', torch.ones(({2})), constraint=constraints.positive)\n".format(
                    argname, argname, dimn)
            elif arg["type"] == "simplex":
                self.guide += self.guide_tab + "{0} = pyro.param('{1}', torch.ones(({2}))/{2}, constraint=constraints.simplex)\n".format(
                    argname, argname, dimn)
            else:
                self.guide += self.guide_tab + "{0} = pyro.param('{1}', torch.ones(({2})))\n".format(argname, argname,
                                                                                                     dimn)
            args.append(argname)
            if 'pyro_args' in posterior:
                arglist += posterior['pyro_args'][index]['name'] + "=" + argname + ","
            else:
                arglist += argname + ","
            index += 1
    ...
    def enterIf_stmt(self, ctx):
        self.model += self.tab + "if {0}:\n".format(PyroVisitor().visit(ctx.expr()))
        self.tab += "    "
    ...
    def enterObserve(self, ctx=Template2Parser.ObserveContext):
        ...
        if len(ctx.expr()) > 1:
            ...
            self.model += self.tab + 'pyro.sample(\'obs_{0}_{4}\'.format({1}), {2}, obs={3})\n'.format(
                ...
                ','.join(self.obs_index),
                PyroVisitor().visit(ctx.expr(0)),
                PyroVisitor().visit(ctx.expr(1)),
                self.nextint)
        else:
            if 'binomial' in PyroVisitor().visit(ctx.distexpr()).lower():
                convdouble = ".float()"
            elif 'bernoulli' in PyroVisitor().visit(ctx.distexpr()).lower():
                convdouble = ".float()"  
            else:
                convdouble = ".float()"  
            convdouble = ""
            self.model += self.tab + 'pyro.sample(\'obs_{0}_{4}\'.format({1}), {2}, obs={3})\n'.format(
                '_'.join(self.obs_string),
                ','.join(self.obs_index),
                PyroVisitor().visit(ctx.distexpr()),
                PyroVisitor().visit(ctx.expr(0)) + convdouble,
                self.nextint)
        self.nextint += 1
        ...
        for data_key in self.data.keys():
            ...
            if isinstance(data_item, np.ndarray):
                ...
                if data_item.ndim == 1:
                    ...
                    self.output_program += data_key + "= np.array(" + str(data_item.tolist()) + \
                                           ", dtype={1}).reshape({0},1)\n".format(str(data_item.shape[0]), datatype)
                else:
                    self.output_program += data_key + "= np.array(" + str(data_item.tolist()) + ", dtype={0})\n".format(
                        datatype)
                self.output_program += data_key + "=torch.tensor(" + data_key + ")\n"
            else:
                self.output_program += data_key + "=" + str(data_item) + "\n"
                if type(data_item) == np.int:
                    self.output_program += data_key + "=torch.tensor(" + data_key + ", dtype=torch.int64)\n"
                else:
                    self.output_program += data_key + "=torch.tensor(" + data_key + ")\n"
        ...
        if self.algo == 'SVI':
            ...
            if self.posteriors is not None:
                for k in self.posteriors.keys():
                    postreturn+= "\"{0}\": {0},".format(k)

            self.output_program += "    pass\n"
            if len(postreturn) > 0:
                self.output_program += "    return {{ {0} }}\n".format(postreturn)

            
            learning_rate = [0.1, 0.05, 0.01, 0.001, 0.025]
            if self.learning_rate is None:
                self.output_program += "optim = Adam({{'lr': {0} }})\n".format(np.random.choice(learning_rate))
            else:
            ...
            self.output_program += "    if ((i % 200) == 0):\n"
            self.output_program += "        print('[{0}] iter {1} :: {2}'.format(int(time.time() * 1000), i, loss))\n"
            ...
            if self.samples and self.posteriors is not None and len(self.posteriors.keys()) > 0:
                self.output_program += "np.set_printoptions(threshold=np.inf)\n"
                self.output_program += "with open('samples','w') as samplefile:\n"
                for p in self.posteriors:
                    self.output_program += "    samplefile.write('{0}:')\n".format(p)
                    self.output_program += "    samplefile.write(np.array2string(np.array([guide({0})['{1}'].data.numpy() " \
                                           "for _ in range(1000)]), separator=',').replace('\\n',''))\n".format(','.join(self.data.keys()), p)
                    self.output_program += "    samplefile.write('\\n')\n"
