    ...
    public static boolean Transform(String testfile, boolean detmode, MyLogger logger, String testdir, TestFile testFile) {
        ...
        if (detmode) {
            try {
                ...
                for (int l = 0; l < dists; ) {
                    Template2Parser parser = Util.getTemplateParserFromString(filecontent);
                    ...
                    if (distributionNormalizer.modifiedTree &&
                            Util.runTransformedCode(filecontent,
                                    newContent,
                                    Util.TemplateTransformationMap.inverse().get(DistributionNormalizer.class),
                                    logger,
                                    testFile)) {
                        ...
                        Util.removeReplacedDist(testfile, distributionNormalizer.replacedCtx);
                        l++;
                    }
                }
        ...
        if (ruleIndex == 0) {
            try {
                if (ctx.getText().contains("1234.0")) {
                    ...
                    return;
                } else if (Util.checkIfReplaced(this.filename, ctx)) {
                    print("Already replaced distribution... skipping... " + ctx.getText());
                    return;
                }

                print("Removing " + ctx.ID().getText() + " dist: " + Util.getStringFromContext(ctx) + " at line : " + ctx.getStart().getLine());
