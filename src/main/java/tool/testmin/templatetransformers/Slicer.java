        ...
        if (detmode) {
            try {
                ...
                for (int l = assign_stmt; l > 0; ) {
                    ...
                    Slicer slicer = new Slicer(Util.getTemplateParserFromString(filecontent), logger);
                    ...
                    if (assignToRemove.contains(l) || slicer.UnexpectedAssign.contains(l)) {
                        AssignmentRemover assignmentRemover = new AssignmentRemover(
                                Util.getTemplateParserFromString(filecontent),
                                true,
                                l, logger);
                        ...
                        if (assignmentRemover.isModified() &&
                                Util.runTransformedCode(filecontent,
                                        newContent,
                                        Util.TemplateTransformationMap.inverse().get(Slicer.class),
                                        logger,
                                        testFile)) {
            ...
            try {
                ...
                for (int l = observe; l > 0; ) {
                    ...
                    if (observeToRemove.contains(l)) {
                        ObserveRemover observeRemover = new ObserveRemover(Util.getTemplateParserFromString(filecontent),
                                true,
                                l,
                                logger);
