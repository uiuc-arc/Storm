    ...
    public void template_minimize_greedy_det(boolean onlyBasic, boolean enableAnalysis) {
        ...
        while (gchanged) {
            ...
            for (int t = 0; t <= 15; t++) {
                ...
                int transformation = Util.order.get(t);
                if (onlyBasic && Util.isDomain(transformation)) {
                ...
                if (!Util.TemplateTransformationMap.containsKey(transformation))
                    continue;

                logger.print("Running transformer: " + Util.TemplateTransformationMap.get(transformation).getCanonicalName(), true);
        ...
        Util.TimingMap.put(testfile, timediff);
