...
public class DistributionChooser {
    ...
    public DistributionChooser(MyLogger logger) {
        this.models = Util.getDistributions(TestMinimizer.pps);
        this.logger = logger;
    ...
    public String getSupportingDistrib2(String currentDistribution, StanParser.Distribution_expContext ctx) {
        ...
        for (JsonObject model : this.models) {
            try {
                if (model.containsKey("name") && includes(model.getString("support"), support)) {
                    candidate_distribs.add(model);
                }
            } catch (Exception e) {
                continue;
    ...
    private JsonObject chooseDistrib(ArrayList<JsonObject> candidates) {
        ...
        if (Util.DistributionChoice.equals("TIMING")) {
            String[] timingfile = new String[0];
            try {
                timingfile = new String(Files.readAllBytes(Paths.get(Util.TimingFile))).split("\n");
            } catch (IOException e) {
                e.printStackTrace();
            }
        ...
        } else if (Util.DistributionChoice.equals("EXPONFAMILY")) {
    ...
    public boolean includes(String candidate_support, String support) {
