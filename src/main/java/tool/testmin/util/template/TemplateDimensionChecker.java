...
public class TemplateDimensionChecker extends TemplateBaseTransformer {
    ...
    public static Map<String, Dimension> getDimensionMap(String program) {
        TemplateDimensionChecker dimensionChecker = new TemplateDimensionChecker(Util.getTemplateParserFromString(program));
    ...
    public void enterPrior(Template2Parser.PriorContext ctx) {
        ...
        String type = null;
        List<JsonObject> dists = Util.getDistributions(TestMinimizer.pps);
        for (JsonObject model : dists) {
            try {
                if (model.getString("name").equals(currentDistribution)) {
                    if (Util.isInteger(model.getString("support"))) {
                        type = "int";
                    } else {
                        type = "real";
                    }
        ...
        dimensionMap.put(Util.getParamName(ctx), dimension);
    }
