...
curpath=`realpath dyes.template`
here=`realpath .`
initcontent=`cat dyes.py`
if [ -z $2 ]
then
(cd ../../../../translators && ./test.py $curpath -a SVI && mv dyes.py $here/dyes.py) > output 2>&1
else
(cd ../../../../translators && ./test.py $curpath -a SVI -it $2 && mv dyes.py $here/dyes.py) > output 2>&1
fi
if [ $? -ne 0 ]; then
